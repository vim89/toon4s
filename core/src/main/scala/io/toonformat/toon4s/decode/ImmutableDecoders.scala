...
/** Immutable decoders using ImmutableLineCursor.
  ...
  * of mutable [[LineCursor]]. All operations return new cursor states, making the code
  * referentially transparent and easier to reason about.
  *
  * ==Strategy==
    ...
    inline match {
      case Some(values) =>
        ...
        val elements = tokens.map(
          t => parsePrimitiveWithValidation(t.trim, options)
        )
