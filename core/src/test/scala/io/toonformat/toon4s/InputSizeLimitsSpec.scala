...
""".trim
    ...
    result.left.foreach {
      err => assert(err.message.contains("nesting depth"), s"Error should mention depth: ${err.message}")
    }
  }
...
""".trim
  ...
  test("rejects arrays exceeding maxArrayLength") {
    ...
    result.left.foreach {
      err => assert(err.message.contains("array length"), s"Error should mention array length: ${err.message}")
    }
  }
  ...
  test("rejects strings exceeding maxStringLength") {
    ...
    result.left.foreach {
      err => assert(err.message.contains("string length"), s"Error should mention string length: ${err.message}")
    }
  }
