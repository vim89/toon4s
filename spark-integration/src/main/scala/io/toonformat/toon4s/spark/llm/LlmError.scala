 ...
 *
 * @see [[https://github.com/llm4s/llm4s llm4s error package]]
 */
...
object LlmError {
   ...
   * @param violations List of specific violations
   */
  final case class ValidationError(
    ...
    override val context: Map[String, String] = field.map(f => Map("field" -> f)).getOrElse(Map.empty)
  }
   ...
   *
   * @param message Configuration issue description
...
object LlmClientHelpers {
   ...
   * @param baseDelayMillis Base delay for exponential backoff
   * @param operation Operation to retry
   * @return Either final error or success value
   */
  def retry[A](
    maxAttempts: Int,
   ...
   * @param userContent User content (may include TOON data)
